...
class Certification extends Component {
    ...
    updateUserInfo = (golds, level, candyP) => {
        ...
        setTimeout(() => {
            ...
            this.props.resetUserInfo(userInfoServer);
        }, 1000);
    }
    /**
	 * 提交
    ...
    getZimid = () => {
        Keyboard.dismiss();
        let { username, alipay, alipayTemp, idCard } = this.state;
        ...
        Send("api/User/FaceInit", { certName: username, certNo: idCard, alipay: alipay }).then(response => {
        ...
        //             // certifyId
        //             this.setState({
        //                 certifyId: responseJson.data.certifyId
        //             })
        //             // 调用刷脸SDK
        //             this.verify(responseJson.data.certifyUrl, responseJson.data.certifyId);
        //         } else {
        ...
        if (this.props.auditState == 1) {
            return (
                <View style={{ alignItems: 'center' }}>
                    <TouchableOpacity onPress={this.getZimid}>
    ...
    render() {
        return (
            <View style={Styles.container}>
                <Header title="实名认证"  rightStyle={{ fontSize: 12 }} onRightPress={() => { this.onRightPress() }} />
                <View style={{ flex: 1 }}>
