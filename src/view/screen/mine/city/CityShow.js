                    ...
                    {SERVICE_BAR.map(item =>
                        <TouchableWithoutFeedback key={item['key']} onPress={() => this.handleServiceYoBang(item)}>
                            <View style={Styles.barBodyItem}>
                                <Text style={Styles.barText}>{this.props.userId == this.state.cityData.userId ? item['title'] == "糖果收益" ? this.state.cityData.candyEarnings : item['title'] == "现金收益" ? this.state.cityData.cashEarnings : item['title'] == "城市人数" ? this.state.cityData.people : 0 : "--"}</Text>
                ...
                <ScrollView style={{paddingHorizontal: 5}}>
                    <TouchableOpacity style={Styles.labelContainer} onPress={() => this.jumpToCityDivideList(1, 1)}>
                        ...
                        <View style={{ flex: 1, flexDirection: 'row', justifyContent: 'flex-end', alignItems: "center" }}>
                            <Icon name="arrow-forward" style={{ color: Colors.main, fontSize: 20 }} />
                        </View>
                    </TouchableOpacity>
                    <TouchableOpacity style={Styles.labelContainer} onPress={() => this.jumpToCityDivideList(1, 2)}>
                        <Text style={Styles.lableTxt}>哟帮糖果分红「{`${this.props.userId == this.state.cityData.userId ? this.state.cityData.yoBangCandy : "--"}`}」</Text>
                        <View style={{ flex: 1, flexDirection: 'row', justifyContent: 'flex-end', alignItems: "center" }}>
                            <Icon name="arrow-forward" style={{ color: Colors.main, fontSize: 20 }} />
                        </View>
                    </TouchableOpacity>
