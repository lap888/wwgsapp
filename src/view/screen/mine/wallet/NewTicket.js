...
export default class NewTicket extends Component {
    ...
    exchangeTicket = () => {
        const data = this.refs.ticketItem.state.selected;
        if (data == '') {
        ...
        Alert.alert(
            ...
            [
                { text: "确定", onPress: () => {
                    UserApi.exchangeTicket({shares: data.shares})
                    .then((data) => {
                        Toast.tip('兑换成功')
                        this.getTiketInfo()
                    }).catch((err) => console.log('err', err))
    ...
    render() {
        return (
            <View style={{flex: 1, backgroundColor: Colors.backgroundColor}}>
                ...
                {this.renderHeader()}
                <TicketItem ref='ticketItem' data={this.state.list}/>
                <Text style={{marginLeft: 15, marginTop: 8, color: Colors.grayFont}}>{this.state.data.rules}</Text>
                <BigButton style={{marginTop: 20}} name={'立即兑换'} onPress={this.exchangeTicket}/>
            </View>
        );
    }
}
