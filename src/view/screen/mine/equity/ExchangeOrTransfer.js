...
export default class ExchangeOrTransfer extends Component {
	...
	btn = () => {
		...
		if (this.state.type == 'exchange') {
            if (equityNum == '') {
                Toast.tipTop('请输入对换数量')
                return ;
            }
            if (pwd1 == '') {
                Toast.tipTop('请输入您的支付密码')
                return ;
            }
            this.props.exchange(this.state.equityNum, this.state.pwd1)
		}if (this.state.type == 'transfer') {
            if (transferNum == '') {
                Toast.tipTop('请输入转让数量')
                return ;
            }
            if (phone == '') {
                Toast.tipTop('请输入转入的手机号')
                return ;
            }
            if (pwd2 == '') {
                Toast.tipTop('请输入您的支付密码')
                ...
                {type == 'exchange' ? <View style={{paddingHorizontal: 10}}>
                    <View style={styles.inputView}>
                        <TextInput
                            ...
                            placeholder={`兑换股权数量 (${data.unitPrice ? data.unitPrice  : ''}糖果${data.unitPrice ? data.unitPrice : ''}果皮兑换1股)`}
